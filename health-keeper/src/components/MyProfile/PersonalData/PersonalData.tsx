...
const PersonalData = () => {
  ...
  if(userData.personalData?.birthday){ 
	setConvertedBirthday(dateConvert(userData.personalData?.birthday, "normal") );
  setConvertedBirthdayReverse(dateConvert(userData.personalData?.birthday, "reverse") )
  }
  setFullname(`${userData.personalData?.name} ${userData.personalData?.lastName}`)
...
const renderForm = () => {
    if(editFormType === "Imię i nazwisko"){
      return (
      <>
      <input type="text" name="name" defaultValue={userData.personalData?.name}/><br/>
      <input type="text" name="lastName" defaultValue={userData.personalData?.lastName}/>
      </>)
      }else if(editFormType === "Płeć"){
      return(
        userData.personalData?.gender == "mężczyzna" ?
      ...
      return <input type="date" name="editData" defaultValue={convertedBirthdayReverse}/>

    }else if(editFormType === "PESEL"){
      return <input type="text" name="editData" defaultValue={userData.personalData?.PESEL} size={11} />
    }else if(editFormType === "Telefon"){
      return <input type="text" name="editData" defaultValue={userData.personalData?.tel} size={9} />
    }else if (editFormType === "Adres"){
      return <input type="text" name="editData" defaultValue={userData.personalData?.address}/>
    }else if (editFormType === "Adres e-mail"){
      return <input type="text" name="editData" defaultValue={userData.personalData?.email}/>
    }
}
 ...
 const handleSubmit = async (event: React.FormEvent) => {
  ...
  return(
    ...
    <div className={styles.personaldata_container}>
        ...
        <button className={ label === "contactdata" ? styles.personaldata_label_active : styles.personaldata_label} onClick={() => setLabel("contactdata")} >Dane kontaktowe</button>
        {label == "personaldata" ?
        <>
          <div className={styles.data_container}>
            <span className={styles.data_header}>Imię i nazwisko</span>
            <span className={styles.data_detail}>{fullname}</span>
            <button onClick={() => formEdit("Imię i nazwisko", "fullname")}></button>
         </div>
         <div className={styles.data_container}>
            <span className={styles.data_header}>Płeć</span>
            <span className={styles.data_detail}>{userData.personalData?.gender}</span>
         ...
         <div className={styles.data_container}>
            ...
            <button onClick={() => formEdit("Data urodzenia", "birthday")}></button>
         </div>
         <div className={styles.data_container}>
            <span className={styles.data_header}>PESEL</span>
            <span className={styles.data_detail}>{userData.personalData?.PESEL}</span>
         ...
         <div className={styles.data_container}>
            ...
            <span className={styles.data_detail}>{userData.personalData?.tel ? `+48 ${userData.personalData?.tel}` : ""}</span>
            <button onClick={() => formEdit("Telefon", "tel")}></button>
         </div>
         <div className={styles.data_container}>
            <span className={styles.data_header}>Adres</span>
            <span className={styles.data_detail}>{userData.personalData?.address}</span>
            <button onClick={() => formEdit("Adres", "address")}></button>
         </div>
         <div className={styles.data_container}>
            <span className={styles.data_header}>Adres e-mail</span>
            <span className={styles.data_detail}>{userData.personalData?.email}</span>
            <button onClick={() => formEdit( "Adres e-mail", "email")}></button>
         </div>
         </>
        }
    </div> :
